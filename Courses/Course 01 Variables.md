...
- **Compile:** the process of the compiler.
...
local + -- Bad ❌: You can't have a variable name include an operator.
...
```lua
-- Bad ❌: Uses "Hello" twice for the same purpose, should be put in a variable.
print("Hello")
print("Hello")
...
```lua
-- Bad ❌: C isnt being used here despite being defined, so it should be removed or used.
local A = "A"
local B = "B"
local C = "C"
